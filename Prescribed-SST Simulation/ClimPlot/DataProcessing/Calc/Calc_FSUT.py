...
def Calc(ModelProperties):
	...
	if not os.path.exists(Data_Path + '/FSUT/'):
		
		os.makedirs(Data_Path + '/FSUT/')
	...
	for i_Run in ['CTR', 'DEF']:
		...
		for i_En in ModelProperties['Ensemble']['Default_En']:
			...
			New_FilePath = \
			Data_Path + '/FSUT/{ModelName}_{En}_FSUT_{Run}.nc'.format(ModelName=ModelProperties['ModelName'], En=str(i_En).zfill(ModelProperties['Ensemble']['Default_En_Format']['ZeroFill']), Run=i_Run)
			...
			# Calculate FSUT = SOLIN - FSNT
			Data['FSUT'] = Data['SOLIN'] - Data['FSNT']
			...
			Create_NewVar               = New_File.createVariable('FSUT', 'f8', ('time', 'lat', 'lon'))
			Create_NewVar.standard_name = 'FSUT'
			Create_NewVar.long_name     = 'Upwelling shortwave flux at top of model'
			Create_NewVar.units         = 'Wm-2'
			Create_NewVar[:]            = Data['FSUT']
