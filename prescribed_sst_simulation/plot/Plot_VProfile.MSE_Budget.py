"""
Plot_VProfile.MSE_Budget.py
==========================
Plot vertical profiles for the MSE budget.
If the variables are advection terms and divergence terms, the sign is reversed (i.e. dV, Vdhm and Omegadhm are multiplied by -1).
"""
...
def Plot_VProfile(Plot_Data, Plot_Config):
	...
	# Plot save
	plt.tight_layout()
	Figure_Path = '../output/Output_Figure/Plot_VProfile.MSE_Budget/'
	Figure_Name = 'Plot_VProfile.MSE_Budget.{Var}.{Run}.png'.format(Var=Plot_Config['Var'], Run=Plot_Config['Run'])
...
def Get_xticks(Run, Var):

	if (Run == 'CTL'):

		if (Var == 'dV'):

			xticks = np.arange(-4e-6, 6e-6, 2e-6)
		
		elif (Var == 'Omega'):

			xticks = np.arange(-0.06, 0.09, 0.03)
		
		elif (Var == 'Vdhm'):

			xticks = np.arange(-0.004, 0.006, 0.002)
		
		elif (Var == 'Omegadhm'):

			xticks = np.arange(-0.02, 0.03, 0.01)
		...
		if (Var == 'dV'):
			...
			xticks = np.arange(-0.01, 0.015, 0.005)
...
if (__name__ == '__main__'):
	...
	for i_Var in ['dV', 'Omega', 'Vdhm', 'Omegadhm']:

		for i_Run in ['CTL', 'ANO', 'ANO_ThDyn', 'ANO_Dyn', 'ANO_NL']:
			...
			# Change sign
			if (i_Var in ['dV', 'Vdhm', 'Omegadhm']): Data = -Data
			
			# Plot: VProfile
			Plot_Data = {\
				'Mean_Group1'      : Prep.Calc_CI(Data[Mask_Group1, :], Data_Type='General', Test_Axis=0)[0], \
				'CI_Group1'        : Prep.Calc_CI(Data[Mask_Group1, :], Data_Type='General', Test_Axis=0)[1], \
				'Mean_Group2'      : Prep.Calc_CI(Data[Mask_Group2, :], Data_Type='General', Test_Axis=0)[0], \
				'CI_Group2'        : Prep.Calc_CI(Data[Mask_Group2, :], Data_Type='General', Test_Axis=0)[1], \
			...
			Plot_Config = {\
				'Plot_xticks': Get_xticks(i_Run, i_Var), \
				'Var': i_Var, \
				'Run': i_Run, \
			}
